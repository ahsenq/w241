...
dt <- fread("./data/Clingingsmith.2009.csv")
```
...
```{r}
ATE <- dt[ , .('group_mean' = mean(views)), keyby = .(success)] %>%
    .[ , diff(group_mean)]
ATE
```
**We see an ATE of about 0.475, meaning those who win the visa lottery see an average delta in views of about 0.475 versus those that do not win the visa lottery.**
...
sum(abs(res) >= ATE)
```
**Of the simulated random assignments, 35 generate an estimated ATE whose absolute value is at least as large as the ATE we estimated.**
...
mean(abs(res) >= ATE)
```
**The implied two-tailed p-value is 0.0035.**
...
d2 %>%
  ...
  summarize(mean_bills = mean(bills_introduced))
```
**We see that the difference in bills introduced by each state is lower when grouped by term. However, if you normalized the data using time, 2 year terms produce more bills as per the data**

**This does seem to suggest that more bills get passed during two year terms.**
...
```
**As you can see the treatment effect under the sharp null is very close for both states.**
